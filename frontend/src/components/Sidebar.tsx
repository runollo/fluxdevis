...
const NAV = [
  ...
  { href: "/factures", label: "Factures", icon: "M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" },
];
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile : header titre + engrenage parametres */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-[#1A355E] text-white flex items-center justify-center h-12">
        ...
        <Link href="/parametres" aria-label="Parametres"
          className={`absolute right-3 top-1/2 -translate-y-1/2 ${pathname.startsWith("/parametres") ? "text-white" : "text-white/70"}`}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d={GEAR} />
      ...
      <aside className="hidden lg:flex lg:w-56 lg:flex-col lg:fixed lg:inset-y-0 bg-[#1A355E] text-white">
        ...
        <div className="p-2 border-t border-white/10">
          <Link href="/parametres"
            className={`flex items-center gap-3 px-3 py-2 rounded text-sm transition-colors ${pathname.startsWith("/parametres") ? "bg-white/15 font-medium" : "hover:bg-white/10"}`}>
            <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d={GEAR} />
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
